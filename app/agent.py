...
# ── Try ADK 2.0 Workflow graph, fall back to single Agent ────────────────────
try:
    from google.adk.agents import Agent, Workflow
    from google.adk.workflow import Edge
    ADK_WORKFLOW = True
    print("ADK 2.0 Workflow graph enabled")
except ImportError:
    from google.adk.agents import Agent
    ADK_WORKFLOW = False
    print("ADK Workflow not available, using single agent")
...
if ADK_WORKFLOW:
    # Full ADK 2.0 Workflow graph — the impressive version
    # Structure and sentiment run first (could be parallel in future)
    # then action extraction, then final summary
    try:
        root_workflow = Workflow(
            name="meeting_intelligence_workflow",
            edges=[
                ("START", structure_agent, action_agent, summary_agent),
            ]
        )
        app = root_workflow
        print("Workflow graph created successfully")
    except Exception as e:
        print(f"Workflow creation issue: {e}, using single agent")
        ADK_WORKFLOW = False
